...
const ModalComponent = ({ modalVisable, setModalVisible, track, onStop }) => {
  ...
  const togglePlayback = async () => {
    ...
    const setupPlayer = async () => {
      try {
        /*
        Trackplayer kütüphanesinin oynatıcıyı kurmasını sağlar. Bu işlemde oynatıcıyı başlatmak için 
        gerekli olan yapılandırmayı yaptık.
        */
  ...
  <Modal
        ...
        style={{ margin:0 }}
        >
  <View style={{ backgroundColor: '#1aa55e', width: '100%', height: '100%', paddingTop: 80, paddingHorizontal: 10}}>
    ...
    <View style={{ padding:10, marginTop:20}}>
      <Image source={{uri:selectedTrack?.images.coverart}} style={{ width: '100%', height:300, justifyContent: 'center', alignItems: 'center', borderRadius:30}}/>
    </View>
    <View style={{flexDirection: 'row', justifyContent: 'space-between', marginTop:20}}>
      <View style={{padding: 5}}>
        <Text style={{fontSize: 22, fontWeight:'bold', color:'black', paddingBottom:5}}>{' '}{selectedTrack?.title}{' '}</Text>
        <Text style={{fontSize: 18, fontWeight:'bold', color:'white'}}>{' '}{selectedTrack?.subtitle}{' '}</Text>
      </View >
      <AntDesign name='heart' size={28} color='white' style={{padding: 15}}/>
    </View>
    <View style={{marginTop:10}}>
      <View style={{width:'100%', marginTop:10, height: 3, backgroundColor: 'black',
      borderRadius:5,
      }}>
        ...
        <View style={{position: 'absolute', top: -5, width:10, height:10, backgroundColor: 'white', borderRadius:5, 
        ...
        <View style={{flex:1, flexDirection: 'row', justifyContent: 'space-between', marginTop: 18, alignItems: 'center'}}>
          ...
          onPress={togglePlayback}>
           {isPlaying ? (
            <AntDesign name='pausecircle' size={26} color='black' style={{ zIndex: 10 }} />
          ) : (
